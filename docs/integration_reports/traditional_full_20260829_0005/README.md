...
- 自动生成的完整逐轮报告：[final_report.md](final_report.md)
- 全体问题明细：[session_findings.jsonl](session_findings.jsonl)
- 每阶段命令、耗时与退出码：[stages.json](stages.json)
- 运行环境、提交和输入哈希：[manifest.json](manifest.json)
- 已发布文件哈希：[checksums.sha256](checksums.sha256)

完整的 `analysis.json`、两种模式的结果 JSON、逐会话 journal 和原始日志体积较大，继续保存在本机忽略目录 `integration_runs/traditional_full_20260829_0005/`，不提交到 Git。发布目录保留结论、代表性逐层证据、全体问题分类和运行身份，足够进行团队审阅及问题分派。
