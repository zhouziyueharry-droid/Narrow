...
- **结论先说**：`PreciseReranker` 经过五轮迭代——v1（有回归）→ v2（按根因修复，追回大部分但未反超）→ v3（用评测数据拟合权重，在留出集上小幅反超 `FallbackReranker`，+0.008，但统计上不够稳）→ v4（正则化强度调对了，优势扩大到 +0.028，自助法验证明显更稳健）→ **v5（当前默认，用官方50000商品目录反推筛选逻辑、合成500条额外训练场景，和100条真实场景一起拟合，在同一份留出集上又比v4多赢了一点，+0.033 vs 原版基线）**。`orchestration/graph.py` 现在默认使用 `PreciseReranker`，权重就是 v5。详细方法、验证方式和已知局限见第八、十四、十五节，请务必看完局限部分再决定要不要长期用这版权重。
...
## 十五、V5：用官方目录反推筛选逻辑，合成500条训练场景，真正帮上了忙

第十四节的 v4 解决了"正则化强度选错"的问题，但没解决更根本的问题：**拟合权重从头到尾只用了100条真实训练场景**。这一节尝试用官方自带的5万商品目录（`data/catalog.jsonl`）自己造更多训练场景来扩充这100条——第一次尝试失败了（如实记录），第二次找对方法后真的有效，当前默认（v5）就是第二次的结果。

### 15.1 为什么可以"自己造"训练场景

看了 `evaluator/local_evaluator.py` 的实现后发现，官方200条场景（`data/public_set.jsonl`）真正需要人工准备的字段只有4个：`sample_id`、`scenario_type`、`user_profile`、`ground_truth.parent_asin`。最关键的两块——`intent_card`（硬约束+软偏好）和 `behavior`（含 intent_override 的转折逻辑）——是 `materialize_hidden_fields()` 在跑评测时**当场用 `intent_card(product)` 和 `behavior_for(scenario_type, card, rng)` 从目标商品自身的文本（标题/features/details/价格）确定性生成的**，不是人工标注、也不依赖外部数据源。也就是说，任选目录里一个商品当目标、配一个场景类型、编一个 `user_profile`，就能造出一条格式和生成逻辑都和官方一致的新场景，不需要写任何格式转换代码。

### 15.2 第一次尝试：随机抽目标商品，结果更差了

第一次直接从5万商品里随机抽了2000个当目标（只过滤了"有标题+有features或details"），场景类型比例对齐官方（40%买/40%逛/15%改变主意/5%不在乎），跑通完整流程（收集特征、拟合权重、在干净留出集上验证）后结果是负面的：

| 版本 | 干净留出集 TechnicalScore |
|---|---|
| FallbackReranker（原版基线） | 0.634392 |
| v4（100条真实场景拟合，当时的默认） | 0.662590 |
| 只用2000条随机抽样合成数据拟合 | **0.594531**（比原版基线还差） |
| 100条真实+2000条随机合成混合拟合 | 0.612970（同样比v4差） |

排除了是正则化强度没调好的可能（换了几个C值，拟合出来的系数几乎不变）。真正的原因是**造数据时用的对话轮数上限从官方的10轮压到了4轮**（纯粹是为了在设备执行时间限制内跑完2000条），加上**目标商品选取太随机，和官方200条目标商品的真实分布差得很远**。

### 15.3 反推官方200条的筛选逻辑

实际统计了官方200条目标商品 vs 全量5万商品目录的差异：

| 特征 | 官方200条目标商品 | 全量5万商品目录 |
|---|---|---|
| 有价格的比例 | **89.0%** | 21.1% |
| 有features的比例 | 100% | 89.6% |
| features数量中位数 | **8** | 5 |
| **评价数量（rating_number）中位数** | **6846** | **12**（p95分位数才602） |
| 平均评分 | 4.37 | 4.09 |
| 价格区间 | $5.49~$87.99（中位数$24.5） | $0~$4119（且只有21%有价格） |

最关键的一条是评价数量：官方目标商品的评价数中位数是6846，全量目录的p95分位数才602——官方选的目标商品几乎都是目录里**评价数排进前1~2%的热门商品**，不是随便一个冷门单品。第一次造数据完全没管这条，相当于绝大多数抽到的都是评价数个位数、价格缺失的冷门商品，和官方场景的真实分布差得远。

按这几条反推出的筛选条件（有价格 + features≥6 + 评价数≥1000 + 价格在$5~90之间），测了一下5万商品里还剩多少符合：

| 筛选条件 | 剩余商品数 |
|---|---|
| 有价格 + features≥6 + 评价数≥500 | 877 |
| 有价格 + features≥6 + 评价数≥1000 | 614 |
| 有价格 + features≥6 + 评价数≥1000 + 价格$5~90 | **600** |

符合条件的商品只剩600个左右（占全量的1.2%），比第一次用的49760个候选池小了80多倍——意味着造场景时得从这600个里**有放回地重复抽取**（同一个商品配不同场景类型/user_profile造出不同场景），这是必须接受的取舍。

### 15.4 第二次尝试：按筛选后的商品池造500条，对话轮数也改回官方的10轮

用新的筛选条件造了500条合成场景（这次对话轮数也没有再压缩，用回官方的10轮，避免和第一次一样引入"轮数"这个额外的干扰变量），实际跑出来 **428/500（85.6%）的场景能命中目标**，远高于第一次随机抽样的表现，说明这次场景质量确实更接近官方200条。用500条合成数据（以及"100条真实+500条合成"混合）重新拟合权重，在同一份干净留出集上验证：

| 版本 | hit_rate@10 | MRR | MTTC | Efficiency | **TechnicalScore** |
|---|---|---|---|---|---|
| FallbackReranker（原版基线） | 0.81 | 0.301306 | 4.05 | 0.695 | 0.634392 |
| v4（100条真实场景，当时的默认） | 0.82 | 0.383552 | 4.16 | 0.684 | 0.661866 |
| 只用500条筛选后的合成数据 | 0.84 | 0.351417 | 3.85 | 0.715 | 0.668425 |
| **100条真实 + 500条筛选后合成，混合拟合（v5，当前默认）** | **0.83** | 0.368476 | **3.94** | **0.706** | **0.666743** |

**注意这里有个如实记录的细节**：上表"100条真实+500条筛选后合成"这一行的具体数字，和我在对话里最初预览时报的数字（TechnicalScore 0.676105）不完全一样——因为预览时用的是一次性写的临时脚本生成的500条数据，正式写进仓库的 `scripts/generate_synthetic_scenarios.py` 因为随机数调用顺序不同，用同样的随机种子（20260829）实际生成的是**具体内容略有差异的另外500条**（都符合同样的筛选条件，只是抽到的商品/场景类型/profile组合不同）。这个差异本身也是一个值得记录的证据：**同一套筛选逻辑，仅仅因为具体随机抽样结果不同，最终效果就能有百分之一量级的波动**——说明目前这个改进量级还没有大到能完全无视抽样噪声的程度。最终以**实际提交进仓库、任何人都能复现的版本**（`data/synthetic_scenarios_500.jsonl`，即上表数字）为准。

### 15.5 稳健性检验（自助法重采样，5000次）

| 对比 | 差值点估计 | 95%置信区间 | 胜率 |
|---|---|---|---|
| v5 vs FallbackReranker | +0.032351 | [-0.023, +0.087] | 87.3% |
| v5 vs v4（原来的默认） | +0.004877 | [-0.009, +0.022] | 72.1% |

v5 相对 `FallbackReranker` 的优势和 v4 差不多稳健（87.3% vs v4当初的84.1%）；v5 相对 v4 本身的优势比较小、也没那么绝对（胜率72.1%，不算特别扎实，比v4当初打败v3的94.7%弱不少），但点估计和绝大多数重采样都支持"v5 略优于 v4"这个方向，且没有引入新的下行风险（v5相对基线的优势和稳健性都不比v4差），所以采用为新默认。

### 15.6 代码改动

| 文件 | 改动 |
|---|---|
| `scripts/generate_synthetic_scenarios.py` | **新增**。按第15.3节反推出的筛选条件（价格、features数量、评价数量、价格区间，均可命令行参数调整）从目录里挑符合条件的商品，配上官方场景类型比例和复用的 `user_profile` 池，生成新的合成训练场景，格式和 `data/public_set.jsonl` 完全一致 |
| `data/synthetic_scenarios_500.jsonl` | **新增**。`generate_synthetic_scenarios.py` 用固定种子（20260829）生成的500条合成场景，v5就是用这份数据训练出来的，随时可删除重新生成 |
| `scripts/fit_precise_reranker_weights.py` | 新增 `--extra-dataset` 参数，可以把额外的场景文件（比如上面这份合成数据）追加到训练集里一起拟合，永远不会追加到验证集，避免验证时用到自造数据 |
| `src/shopping_agent/ranking/precise.py` | `DEFAULT_WEIGHTS` 换成v5（100条真实+500条合成混合拟合，C=100）的权重；上方注释完整记录了v3→v4→v5的迭代历史、两次合成数据尝试的成败对比、以及15.4节提到的"随机抽样波动"细节 |

复现命令（从 `techjam-conversational-search/` 目录下）：

```bash
# 第一步：生成合成场景（已经生成好并提交了，重新跑会覆盖成新的随机抽样结果）
PYTHONPATH=".:src" python3 scripts/generate_synthetic_scenarios.py \
    --count 500 --output data/synthetic_scenarios_500.jsonl

# 第二步：真实100条 + 合成500条一起拟合，在干净留出集上验证
PYTHONPATH=".:src" python3 scripts/fit_precise_reranker_weights.py \
    --train-slice 100:200 --extra-dataset data/synthetic_scenarios_500.jsonl \
    --val-slice 0:100 --C 100 --output fitted_weights_v5.json
```

（注：`fit_precise_reranker_weights.py` 的 `collect_training_rows()` 是逐场景全量10轮跑的，600个场景在这台设备上实测约6分钟，`main()` 目前没有做分批处理，如果单次执行超时，可以参考本节脚本的思路自己按场景切片分批收集特征、合并后再拟合。）

### 15.7 局限（如实记录）

- **合成数据造出来的候选池只有600个商品**（筛选条件卡得比较严），如果要造比500条明显更多的场景，必然会大量重复使用同一批商品，多样性有限，收益可能会递减甚至反转，需要重新验证而不是假设"越多越好"。
- **15.4节已经证明这个提升幅度对随机抽样很敏感**（同样的筛选逻辑，两次不同的随机抽样结果，v5相对v4的优势从+0.005到+0.014不等）——不建议把当前这版具体数字当作精确值来引用，只能说"这个方向大概率是有效的，幅度不算大"。
- **v5相对v4的胜率只有72.1%**，不如v4当初相对v3那么扎实（94.7%）。如果后面同学重新生成一批新的合成数据（换个随机种子）、拟合出的权重效果比v4还差，也不用意外——这一节的结论是"这个方法论方向是对的"，不是"这个具体数字永远成立"。
- **筛选条件（价格$5~90、评价数≥1000、features≥6）是从官方200条的中位数反推的，不是官方公开的真实标准**，实际筛选逻辑可能更复杂（比如还考虑了类目分布、场景难度分级），这里只是找了几个最容易验证的统计特征去逼近，不代表完全还原了官方的采样方法。
