...
| `techjam_rebuilt_amazon_*` | Catalog-derived non-official session cards | Buying, Browsing, Intent Override, Boundary | Exact `parent_asin`, with override gating | Official-style formulas with `official_metric_contract=false` |
...
user-simulator run --preset techjam_rebuilt_amazon_clothing_50k
user-simulator run --preset techjam_rebuilt_amazon_clothing_200k
user-simulator run --preset techjam_rebuilt_amazon_broad_500k
```

`techjam` uses the official Participant Kit catalog (`catalog.jsonl`, exactly
50,000 products) and official public sessions (`public_set.jsonl`, 200 sessions).
The `techjam_rebuilt_amazon_*` presets use catalogs reconstructed by this project
from Amazon Reviews 2023. In particular, `rebuilt_amazon_clothing_50k.jsonl` is
our new 50K and is not the official Participant Kit 50K. The three rebuilt scale
presets use the same reconstructed core set of 1,000 sessions across nested
catalogs. They never reuse the official 200 conversations or official targets.
Development, challenge, smoke, index, and provenance files are under
`data/derived/techjam_compatible_scale_v1/sessions/`.
